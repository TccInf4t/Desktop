...
public class PedidoSemLoteInicialController implements Initializable{
	...
	public void Filtrar(){
		...
		if(!numPedido.isEmpty()){
			...
			AtualizarTblPedidoComFiltro(Pedido.Filtrar(true, false, false, false, numeroPedido, null, null, null));
			System.out.println(numeroPedido);
		}else if(!dataCompra.isEmpty()){
			if(estado!=null){
				if(cidade!=null){
					//Filtrar pela data realizado e pela cidade
					AtualizarTblPedidoComFiltro(Pedido.Filtrar(false, true, true, true, 0, dataCompra, estado, cidade));
				}else{
					//Filtrar pela data realizado e pelo estado
					AtualizarTblPedidoComFiltro(Pedido.Filtrar(false, true, true, false, 0, dataCompra, estado, null));
				}
			}else{
				//Filtrar pela data realizado
				AtualizarTblPedidoComFiltro(Pedido.Filtrar(false, true, false, false, 0, dataCompra, null, null));
			}
		}else if(estado!=null){
			if(cidade!=null){
				//Filtrar pela cidade
				AtualizarTblPedidoComFiltro(Pedido.Filtrar(false, false, true, true, 0, null, estado, cidade));
			}else{
				//Filtrar pelo estado
				AtualizarTblPedidoComFiltro(Pedido.Filtrar(false, false, true, false, 0, null, estado, null));
			}
		}else{
